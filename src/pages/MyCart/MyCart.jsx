...
const MyCart = () => {
    ...
    const fetchUserCart = async () => {
      try {
        const response = await fetch(
          `https://the-face-store-server-rfsm6nrpx-thuabas-projects.vercel.app/user/cart/${auth.user.email}`
        );
        if (response.ok) {
  ...
  const deleteCartItem = (itemId) => {
    ...
    fetch(
      `https://the-face-store-server-rfsm6nrpx-thuabas-projects.vercel.app/user/cart/${auth.user.email}/${itemId}`,
      {
        method: "DELETE",
      }
    )
  ...
  return (
    <div>
      ...
      <div className="max-w-2xl mx-auto p-4">
        <h2 className="text-2xl text-[#800] font-semibold mb-4">Your Cart</h2>
        <div className="space-y-4">
          {cartItems.map((item) => (
            ...
            >
              ...
              <div className="text-lg text-[#800]">
